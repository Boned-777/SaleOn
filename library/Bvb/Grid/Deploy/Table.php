...
class Bvb_Grid_Deploy_Table extends Bvb_Grid implements Bvb_Grid_Deploy_DeployInterface {
    ...
    protected function _buildPagination()
    {
        ...
        if (1 == $actual) {
            ...
            $pag = "<li><a href=\"$url/start{$this->getGridId()}/0\">1</a></li>";
        }
        ...
        if ($npaginas > 5) {
            ...
            for ($i = $in + 1; $i < $fin; $i++) {
                ...
                if ($i == $actual) {
                    ...
                    $pag .=  " <li><a href='$url/start{$this->getGridId()}/" . (($i - 1) * $ppagina) . "'> $i </a></li>";
                }
            }
            ...
            for ($i = 2; $i < $npaginas; $i++) {
                ...
                if ($i == $actual) {
                    ...
                    $pag .= " <li><a href=\"" . $url . "/start{$this->getGridId()}/"
                         . (($i - 1) * $ppagina) . "\">$i</a> ";
                }
            }
        }
        ...
        if ($actual == $npaginas) {
            $pag .= "<li class='active'><a>" . $npaginas . "</a></li>" ;
        } else {
            $pag .= " <li><a href=\"$url/start{$this->getGridId()}/"
                  . (($npaginas - 1) * $ppagina) . "\">$npaginas</a></li> ";
        }


        if ($actual != 1) {
            $pag = " <li><a href=\"$url/start/0\">" . $this->__('First')
                 . "</a></li><li><a href=\"$url/start{$this->getGridId()}/" . (($actual - 2) * $ppagina)
                 . "\">" . $this->__('Previous') . "</a></li>" . $pag;
        }

        if ($actual != $npaginas) {

            $pag .= "<li><a href=\"$url/start{$this->getGridId()}/" . ($actual * $ppagina) . "\">"
                  . $this->__('Next') . "</a></li><li><a href=\"$url/start{$this->getGridId()}/"
        ...
        if ($npaginas > 1 || count($this->_export) > 0) {
            ...
            if ((int) $this->getInfo("limit") > 0) {
                ...
                $result2 =
                    $this->_temp['table']->pagination("<ul>" . $pag . "</ul>",
                ...
                $result2 .= $this->_temp['table']->pagination("<ul>" . $pag . "</ul>",
                          $this->_totalRecords,
                          $menuPerPage,
                          $pageSelect);
            }
